...
namespace Ragnar.IntegrationDriven.Interest.Calculator.Helpers
{
    ...
    {
        bool Compare<T>(ComparisonAction comparisonAction, T value1, T value2) where T : IComparable;
    }

    public class ComparisonHelper : IComparisonHelper
    {
        public bool Compare<T>(ComparisonAction comparisonAction, T value1, T value2) where T : IComparable
        {
            switch (comparisonAction)
            {
                case ComparisonAction.LessThan:
                    return CompareLessThan(value1, value2);

                case ComparisonAction.LessOrEqualThan:
                    return CompareLessThan(value1, value2) || CompareEqual(value1, value2);

                case ComparisonAction.Equal:
                    return CompareEqual(value1, value2);

                case ComparisonAction.GreaterOrEqualThan:
                    return CompareGreaterThan(value1, value2) || CompareEqual(value1, value2);

                case ComparisonAction.GreaterThan:
